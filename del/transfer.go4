...
func (t *UploadTask) doSendFile(fullPath string) error {
	...
	if err != nil {
		OutputDebugString(fmt.Sprintf("连接失败: %v", err))
	...
	// 发送CHECK命令并读取响应
	if _, err := fConn.Write([]byte(fmt.Sprintf("FILECHECK\n%s|%s|%s|%s|%d\n", t.ClientID, formatUnixPath(fullPath), ct, mt, fileInfo.Size()))); err != nil {
		OutputDebugString(fmt.Sprintf("发送CHECK失败: %v", err))
		return fmt.Errorf("发送CHECK失败: %w", err)
	}

	response, err := bufio.NewReader(fConn).ReadString('\n')
	if err != nil {
		OutputDebugString(fmt.Sprintf("读取CHECK响应失败: %v", err))
		return fmt.Errorf("读取CHECK响应失败: %w", err)
	}

	if strings.TrimSpace(response) == "EXISTS" {
		OutputDebugString(fmt.Sprintf("\033[33m跳过已存在文件: %s\033[0m", fullPath))
		return nil
	}
	...
	if SpeedLimit > 0 {
		...
		if _, err := rateLimitedCopyWithContext(fConn, src, limiter); err != nil {
	...
	// finalize logging
	counter.Finalize()
	OutputDebugString(fmt.Sprintf("\033[32m成功上传: %s (%s)\033[0m\n", fullPath, formatSize(fileInfo.Size())))
	return nil
}
...
func rateLimitedCopyWithContext(dst io.Writer, src io.Reader, limiter *rate.Limiter) (written int64, err error) {
...
func (c *WriteCounter) Write(p []byte) (int, error) {
	...
	// 彩色终端输出
	OutputDebugString(fmt.Sprintf("\r\033[33m%s\033[0m | 进度: \033[32m%.2f%%\033[0m | 速度: \033[36m%.2f KB/s\033[0m", c.FileName, progress, speed))
...
func (c *WriteCounter) Finalize() {
	...
	OutputDebugString(fmt.Sprintf("\r\033[1;32m%s\033[0m | 完成: \033[1;35m100.00%%\033[0m | 平均速度: \033[1;34m%.2f KB/s\033[0m\n", c.FileName, avgSpeed))
}
